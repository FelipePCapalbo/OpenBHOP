...
impl GorgonzoiMesh {
    pub fn new() -> Self {
        ...
        // The larger triangular base is parallel to the ground, with a height/thickness of 0.6
        let height = 0.6;
        let half_height = height / 2.0;

        // Bottom base vertices (Y = -0.3)
        ...
        // Procedural color function to simulate mold patterns on the cheese
        let get_gorgonzola_color = |x: f32, y: f32, z: f32| -> Color {
            // Deterministic noise based on sine/cosine of coordinates
            let noise = (x * 12.5).sin() * (y * 14.2).cos() + (z * 17.3).sin() * (x * 8.9).cos();
            
            if noise > 0.22 {
                Color::new(0.24, 0.38, 0.33, 1.0)   // Dark greenish-blue mold
            } else if noise > 0.05 {
                Color::new(0.42, 0.55, 0.48, 1.0)   // Transition color
            } else {
                Color::new(0.96, 0.93, 0.74, 1.0)   // Base cheese color
            }
        };
            ...
            for i in 0..=latitude_subdivisions {
                ...
                for j in 0..=longitude_subdivisions {
                    ...
                    let color = if dot > 0.88 {
                        Color::new(0.05, 0.05, 0.05, 1.0)   // Pupil
                    } else if dot > 0.70 {
                        Color::new(0.85, 0.45, 0.12, 1.0)   // Iris
                    } else {
                        Color::new(0.98, 0.98, 0.98, 1.0)   // Sclera
                    };
    ...
    pub fn draw(&self, position: Vec3, rotation: Vec3, scale: Vec3) {
        ...
        let model_matrix = Mat4::from_scale_rotation_translation(scale, rotation_quat, position);
